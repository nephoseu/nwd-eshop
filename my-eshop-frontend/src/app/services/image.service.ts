...
export class ImageService {
  ...
  public upload(image: Image): Observable<HttpEvent<Response>> {
    ...
    return this.http.post<Response>(`${environment.apiItemUrl}/image`, formData, {reportProgress: true, observe: 'events'});
  }

  public getImage(itemId:number): Observable<Image> {
    return this.http.get<Image>(`${environment.apiItemUrl}/image/${itemId}`);
  }
}
